...
def render_explanation_section():
    ...
    # Математик тайлбар (Expandable)
    with st.expander(f"{ICONS['info']} SHAP-ийн Математик Онол", expanded=False):
        st.markdown("""
        ### Shapley Утгын Томьёо
        
        SHAP утгыг дараах томьёогоор тооцоолно:
        """)
        ...
        st.markdown("""
        **Тэмдэглэгээ:**
        - $\phi_i$ — $i$-р шинж чанарын SHAP утга
        - $N$ — бүх шинж чанаруудын олонлог
        - $S$ — $i$-г агуулаагүй дэд олонлог
        - $f(S)$ — зөвхөн $S$ шинж чанаруудыг ашигласан загварын гаралт
        
        ### SHAP-ийн Үндсэн Аксиомууд
        
        **1. Локал нарийвчлал (Local Accuracy):**
        """)
        st.latex(r'''f(x) = \phi_0 + \sum_{i=1}^{M} \phi_i''')
        st.markdown("Бүх SHAP утгуудын нийлбэр нь загварын гаралттай тэнцүү.")
        
        st.markdown("**2. Тэгш хуваарилалт (Efficiency):**")
        st.latex(r'''\sum_{i=1}^{M} \phi_i = f(x) - E[f(X)]''')
        st.markdown("SHAP утгуудын нийлбэр нь таамаглал ба дундаж утгын зөрүүтэй тэнцүү.")
        
        st.markdown("**3. Тэгш хэмт байдал (Symmetry):**")
        st.latex(r'''\forall S: f(S \cup \{i\}) = f(S \cup \{j\}) \Rightarrow \phi_i = \phi_j''')
        st.markdown("Ижил нөлөөтэй шинж чанарууд ижил SHAP утгатай.")
        
        st.markdown("""
        ### Жингийн Функц
        
        Shapley томьёолол дахь жингийн утга:
        """)
        st.latex(r'''w(|S|) = \frac{|S|!(|N|-|S|-1)!}{|N|!} = \frac{1}{|N| \binom{|N|-1}{|S|}}''')
        st.markdown("Энэ жин нь $i$ шинж чанар коалицид нэгдэх бүх боломжит дарааллын дунджийг илэрхийлнэ.")
        ...
        | SHAP утга | Тайлбар |
        |-----------|---------|
        | $\phi_i > 0$ | Шинж чанар таамаглалыг **нэмэгдүүлж** байна |
        | $\phi_i < 0$ | Шинж чанар таамаглалыг **бууруулж** байна |
        | $\phi_i \approx 0$ | Шинж чанар **бараг нөлөөгүй** |
        | $|\phi_i|$ их | Шинж чанар **маш чухал** |
        """)
    ...
    if explanations_generated and has_shap_values:
        ...
        if 'global' in explanations:
            ...
            # Математик тайлбар
            with st.expander(f"{ICONS['info']} Глобал Ач Холбогдлын Тооцоо", expanded=False):
                st.markdown("""
                Шинж чанарын глобал ач холбогдол нь бүх дээжүүд дээрх SHAP утгуудын **дундаж абсолют утга**-аар тооцоологдоно:
                """)
                st.latex(r'''I_j = \frac{1}{n} \sum_{i=1}^{n} |\phi_j^{(i)}|''')
                st.markdown("""
                **Тайлбар:**
                - $I_j$ — $j$-р шинж чанарын глобал ач холбогдол
                - $n$ — нийт дээжийн тоо
                - $\phi_j^{(i)}$ — $i$-р дээжийн $j$-р шинж чанарын SHAP утга
                - $|\cdot|$ — абсолют утга (эерэг ба сөрөг нөлөөг ялгахгүйгээр)
                
                Энэ арга нь **permutation importance**-аас илүү найдвартай, учир нь шинж чанаруудын хоорондын корреляцийг тооцдог.
                """)
        ...
        if 'local' in explanations:
            ...
            # Локал тайлбарын математик томьёо
            with st.expander(f"{ICONS['info']} Локал Тайлбарын Математик Үндэслэл", expanded=False):
                st.markdown("""
                ### Таамаглалын Задаргаа
                
                Нэг дээжийн таамаглалыг суурь утга ба шинж чанаруудын хувь нэмэр гэж задалж болно:
                """)
                st.latex(r'''f(x) = \underbrace{E[f(X)]}_{\text{суурь утга}} + \underbrace{\sum_{i=1}^{M} \phi_i(x)}_{\text{шинж чанаруудын нийт хувь нэмэр}}''')
                
                st.markdown("""
                ### Waterfall Тооцоо
                
                Энэ нь загварын таамаглалыг алхам алхмаар тайлбарлах боломж олгоно:
                """)
                st.latex(r'''
                \begin{aligned}
                \text{Суурь утга} &= E[f(X)] = \phi_0 \\
                + \phi_1 &\rightarrow \text{1-р шинж чанарын нөлөө} \\
                + \phi_2 &\rightarrow \text{2-р шинж чанарын нөлөө} \\
                &\vdots \\
                + \phi_M &\rightarrow \text{M-р шинж чанарын нөлөө} \\
                &= f(x) \text{ (эцсийн таамаглал)}
                \end{aligned}
                ''')
                ...
                - **Суурь утга** $\phi_0 = E[f(X)]$: Загварын дундаж гаралт (ямар ч мэдээлэлгүйгээр)
                - **Эерэг SHAP** $\phi_i > 0$: Шинж чанар таамаглалыг суурь утгаас дээшлүүлж байна
                - **Сөрөг SHAP** $\phi_i < 0$: Шинж чанар таамаглалыг суурь утгаас доошлуулж байна
                
                ### Шинж Чанарын Утга ба SHAP
                
                Шинж чанарын бодит утга ($x_i$) ба түүний SHAP утга ($\phi_i$) хоёр өөр зүйл:
                - $x_i$ = Шинж чанарын хэмжсэн утга (жишээ нь: нас = 35)
                - $\phi_i$ = Энэ утга таамаглалд хэр их нөлөөлж байгаа
                """)
...
def render_visualization_section():
    ...
    # Визуализацийн математик үндэслэл
    with st.expander(f"{ICONS['info']} Визуализацийн Математик Үндэслэл", expanded=False):
        st.markdown("""
        ### Глобал Шинж Чанарын Ач Холбогдол
        
        Глобал шинж чанарын ач холбогдол нь бүх дээжүүд дээрх SHAP утгуудын дундаж абсолют утгаар тооцоологдоно:
        """)
        st.latex(r'''I_j = \frac{1}{n} \sum_{i=1}^{n} |\phi_j^{(i)}|''')
        st.markdown("""
        үүнд $I_j$ нь $j$-р шинж чанарын ач холбогдол, $\phi_j^{(i)}$ нь $i$-р дээжийн $j$-р шинж чанарын SHAP утга.
        
        ### Summary Plot (Beeswarm)
        
        Summary plot нь шинж чанар бүрийн SHAP утгын тархалтыг харуулна:
        """)
        st.latex(r'''\text{Position}_{i,j} = (\phi_j^{(i)}, j + \text{jitter}_{i,j})''')
        st.markdown("""
        Өнгө нь шинж чанарын бодит утгын хэвийнчилсэн хувилбарыг илэрхийлнэ:
        """)
        st.latex(r'''\text{Color}_{i,j} = \frac{x_j^{(i)} - \min(x_j)}{\max(x_j) - \min(x_j)}''')
        
        st.markdown("""
        ### Зураасан графикуудын тайлбар
        
        | График | Математик үндэс | Ашиглалт |
        |--------|-----------------|----------|
        | Bar Plot | $I_j = E[|\phi_j|]$ | Глобал ач холбогдол |
        | Waterfall | $f(x) = \phi_0 + \sum \phi_i$ | Локал задаргаа |
        | Dependence | $\phi_j$ vs $x_j$ | Хамаарлын шинжилгээ |
        """)
...
def render_fairness_section():
    ...
    with st.expander(f"{ICONS['info']} Хэмжигдэхүүнүүдийн Математик Тайлбар", expanded=False):
        st.markdown("""
        ### Demographic Parity (Хүн ам зүйн Тэнцвэрт байдал)
        
        Загвар өөр өөр бүлгүүдэд эерэг таамаглал өгөх хувь ижил байх ёстой:
        """)
        st.latex(r'''P(\hat{Y}=1|A=a) = P(\hat{Y}=1|A=b), \quad \forall a, b \in A''')
        st.markdown("""
        үүнд $\hat{Y}$ нь таамаглал, $A$ нь хамгаалагдсан атрибут (жнь: хүйс).
        
        **DP зөрүү:**
        """)
        st.latex(r'''\text{DP Difference} = P(\hat{Y}=1|A=1) - P(\hat{Y}=1|A=0)''')
        st.markdown("Шударга байдалд 0-д ойрхон байх ёстой.")
        
        st.markdown("""
        ### Disparate Impact (Ялгаварт Нөлөө)
        
        Хамгийн бага хувийн бүлэг ба хамгийн их хувийн бүлгийн харьцаа:
        """)
        st.latex(r'''\text{DI} = \frac{P(\hat{Y}=1|A=0)}{P(\hat{Y}=1|A=1)}''')
        st.markdown("""
        - $DI \geq 0.8$ (80%-ийн дүрэм) бол шударга гэж үзнэ
        - $DI < 0.8$ бол алагчлал байж болзошгүй
        
        ### Equalized Odds (Тэнцүүлсэн Магадлал)
        
        TPR (True Positive Rate) ба FPR (False Positive Rate) бүлэг бүрт ижил байх:
        """)
        st.latex(r'''P(\hat{Y}=1|Y=1, A=a) = P(\hat{Y}=1|Y=1, A=b)''')
        st.latex(r'''P(\hat{Y}=1|Y=0, A=a) = P(\hat{Y}=1|Y=0, A=b)''')
        
        st.markdown("""
        ### Equal Opportunity (Тэгш Боломж)
        
        Зөвхөн TPR бүлэг бүрт ижил байхыг шаарддаг (Equalized Odds-ын хялбаршуулсан хувилбар):
        """)
        st.latex(r'''P(\hat{Y}=1|Y=1, A=a) = P(\hat{Y}=1|Y=1, A=b)''')
        
        st.markdown("""
        ### Хэмжигдэхүүний Хүснэгт
        
        | Хэмжигдэхүүн | Шударга утга | Тайлбар |
        |--------------|--------------|---------|
        | DP Difference | $\\approx 0$ | Бүлгүүдийн ялгаа бага |
        | Disparate Impact | $\\geq 0.8$ | 80%-ийн дүрэм хангасан |
        | Equalized Odds Diff | $\\approx 0$ | TPR, FPR ижил |
        """)
